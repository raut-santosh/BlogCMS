...
<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(!empty($username) && !empty($email) && !empty($password)){
            ...
            $registration_user_query = mysqli_query($conn, $query);
            if(!$registration_user_query){
                die("QUERY FAILED ". mysqli_error($conn) . '' . mysqli_errno($conn));
            }
            // Message to print after complete registration
            $message = "Registration is Successfull";
        }else{
            // Message to print after not complete registration
        ...
        // making this empty so that php will not give error if submit btn is not clicked
        $message = "";
    }
